...
Xenia iOS A64 code cache can request external JIT region preparation through:
...
- `tools/ios/xenia_ios_lldb_jit_broker.py`
...
1. Catch `SIGTRAP` stops caused by the Xenia JIT BRK protocol.
...
Then run `xenia-jit-broker-install` once the target is created.
...
```bash
tools/ios/install_lldbinit_xenia_jit.sh
```
